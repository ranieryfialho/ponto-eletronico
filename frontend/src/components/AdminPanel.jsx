    ...
    try {
      ...
      setFormMessage(
        <div>
          ...
          <p className="mt-2 text-xs text-gray-600">Copie e envie este link para o funcionário:</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
          ...
          <form onSubmit={handleCreateUser} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input type="text" placeholder="Nome completo do funcionário" value={newName} onChange={(e) => setNewName(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg" required />
              <input type="email" placeholder="E-mail do funcionário" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg" required />
            </div>
            <button type="submit" disabled={isSubmitting} className="w-full md:w-auto bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg">
                  ...
                  {users.map(user => (
                    <tr key={user.uid} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right space-x-2">
                        <button onClick={() => setReportingUser(user)} title="Ver Relatório" className="p-2 text-gray-500 rounded-full hover:bg-blue-100 hover:text-blue-600">
                        ...
                        <button onClick={() => handleOpenEditModal(user)} title="Editar Funcionário" className="p-2 text-gray-500 rounded-full hover:bg-indigo-100 hover:text-indigo-600">
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                        </button>
                        <button onClick={() => handleRemoveUser(user.uid, user.email)} title="Remover Funcionário" className="p-2 text-gray-500 rounded-full hover:bg-red-100 hover:text-red-600">
