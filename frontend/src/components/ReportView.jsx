// src/components/ReportView.jsx (Versão 100% Completa, Final e Corrigida)
import React, { useState, useEffect, useCallback } from 'react';
import { auth } from '../firebase-config';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';

// Função auxiliar para formatar milissegundos em HH:mm
...
export function ReportView({ user, onBack }) {
  ...
  // Efeito 1: Busca o perfil do funcionário (com a jornada de trabalho)
  useEffect(() => {
  ...
  // Função para gerar o relatório principal
  const handleGenerateReport = useCallback(async () => {
    ...
    try {
      const token = await auth.currentUser.getIdToken();
      const response = await fetch(`http://localhost:3001/api/admin/reports/time-entries?userId=${user.uid}&startDate=${startDate}&endDate=${endDate}`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
      ...
      for (const date in entriesByDay) {
        ...
        entriesByDay[date].punches.forEach(punch => {
          const entryTime = punch.time;
          if (punch.type === 'Entrada') clockInTime = entryTime;
          if (punch.type === 'Início do Intervalo') breakStartTime = entryTime;
          if (punch.type === 'Fim do Intervalo' && breakStartTime) { entriesByDay[date].totalBreakMillis += entryTime - breakStartTime; breakStartTime = null; }
          if (punch.type === 'Saída' && clockInTime) { entriesByDay[date].totalWorkMillis += entryTime - clockInTime; clockInTime = null; }
        });
  ...
  // Efeito 2: Gera o relatório quando o perfil ou as datas mudam
  useEffect(() => {
    if (employeeProfile) { // Só gera o relatório se o perfil já foi carregado
    ...
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      <div className="p-6 bg-white rounded-xl shadow-md border border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div><label htmlFor="start-date" className="block text-sm font-medium text-gray-700">Data de Início</label><input id="start-date" type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm" /></div>
          <div><label htmlFor="end-date" className="block text-sm font-medium text-gray-700">Data de Fim</label><input id="end-date" type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm" /></div>
          <div className="flex gap-2">
            <button onClick={handleExportCSV} disabled={!reportData || isLoading} className="flex-1 bg-teal-600 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded-md h-11 flex items-center justify-center gap-2 disabled:bg-gray-400"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>Excel</button>
            <button onClick={handleExportPDF} disabled={!reportData || isLoading} className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-md h-11 flex items-center justify-center gap-2 disabled:bg-gray-400"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>PDF</button>
          </div>
        </div>
