...
const formatMillisToHours = (millis, allowNegative = false) => {
  ...
  const sign = millis < 0 ? '-' : '';
  if (allowNegative) { millis = Math.abs(millis); }
  else if (millis < 0) { millis = 0; }
...
const getExpectedWorkMillis = (dayOfWeek, schedule) => {
  ...
  let daySchedule;
  if (dayOfWeek === 6 && schedule.saturday?.isWorkDay) { daySchedule = schedule.saturday; }
  else if (dayOfWeek >= 1 && dayOfWeek <= 5) { daySchedule = schedule.weekday; }
  else { return 0; }
...
function App() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setEmployeeProfile(profileData);
           ...
           console.error("Falha ao buscar perfil do funcionário.");
           setEmployeeProfile(null);
        }
      } catch(error) {
  ...
  const groupedHistory = useMemo(() => {
    if (!employeeProfile?.workHours) return {};
    ...
    const groups = timeHistory.reduce((acc, entry) => {
      ...
      if (!acc[monthYear]) { acc[monthYear] = { days: {}, totalBalanceMillis: 0 }; }
      if (!acc[monthYear].days[day]) { acc[monthYear].days[day] = []; }
    ...
    for (const month in groups) {
        ...
        for (const day in groups[month].days) {
            ...
            sortedPunches.forEach(punch => {
                ...
                if (punch.type === 'Fim do Intervalo' && breakStartTime) { dailyBreak += punchTime - breakStartTime; breakStartTime = null; }
                if (punch.type === 'Saída' && clockInTime) { dailyWork += punchTime - clockInTime; clockInTime = null; }
            });
  ...
  const renderActionButtons = () => {
    ...
    switch (userStatus) {
      case STATUS.CLOCKED_OUT: return <button onClick={() => handleRegister(ENTRY_TYPES.CLOCK_IN)} className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-6 rounded-lg shadow-sm transition-colors">▶️ Iniciar Expediente</button>;
      case STATUS.WORKING: return (<div className="flex gap-4"><button onClick={() => handleRegister(ENTRY_TYPES.BREAK_START)} className="flex-1 bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-3 px-6 rounded-lg shadow-sm transition-colors">⏸️ Iniciar Intervalo</button><button onClick={() => handleRegister(ENTRY_TYPES.CLOCK_OUT)} className="flex-1 bg-red-500 hover:bg-red-600 text-white font-bold py-3 px-6 rounded-lg shadow-sm transition-colors">⏹️ Encerrar Expediente</button></div>);
      case STATUS.ON_BREAK: return <button onClick={() => handleRegister(ENTRY_TYPES.BREAK_END)} className="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-6 rounded-lg shadow-sm transition-colors">▶️ Retornar do Intervalo</button>;
    ...
    return (
      <div className="w-full max-w-md">
        <div className="p-6 bg-white rounded-xl shadow-lg border border-gray-200">
          <p className="mb-4 capitalize text-xl text-center font-bold text-gray-800">Bem-vindo, {user.displayName || user.email}! 👋</p>
          {isAdmin && (<button onClick={() => setShowAdminPanel(true)} className="w-full mb-4 bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-lg shadow-sm transition-colors">Painel do Administrador</button>)}
          <div className="text-lg text-gray-700 mb-6 p-4 h-20 flex items-center justify-center bg-gray-50 rounded-lg border">{message}</div>
          <div className="space-y-4">{renderActionButtons()}</div>
          <button onClick={handleLogout} className="w-full mt-4 flex items-center justify-center gap-2 bg-white border border-gray-300 text-gray-700 font-medium py-2 px-4 rounded-lg hover:bg-gray-100 transition-colors shadow-sm">
        ...
        {Object.keys(groupedHistory).length > 0 && (
          <div className="mt-8 w-full">
            <h2 className="text-2xl font-bold mb-4 text-gray-800">Seu Histórico Recente</h2>
                      ...
                      <>
                        <Disclosure.Button className="flex w-full justify-between items-center rounded-lg bg-blue-100 px-4 py-3 text-left text-sm font-medium text-blue-900 hover:bg-blue-200 focus:outline-none focus-visible:ring focus-visible:ring-blue-500/75 transition-colors">
                          ...
                          <div className='flex items-center gap-2 sm:gap-4'>
                            <span className={`text-xs font-bold px-2 py-1 rounded-full ${isNegative ? 'bg-red-200 text-red-800' : 'bg-green-200 text-green-800'}`}>
                              Saldo: {formatMillisToHours(monthData.totalBalanceMillis, true)}
                            </span>
                            <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 text-blue-500 transition-transform ${open ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        ...
                        <Transition enter="transition duration-100 ease-out" enterFrom="transform scale-95 opacity-0" enterTo="transform scale-100 opacity-100" leave="transition duration-75 ease-out" leaveFrom="transform scale-100 opacity-100" leaveTo="transform scale-95 opacity-0">
                          <Disclosure.Panel className="px-2 sm:px-4 pt-4 pb-2 text-sm text-gray-600 space-y-4">
                            {Object.entries(monthData.days).sort(([dayA], [dayB]) => new Date(dayB.split('/').reverse().join('-')) - new Date(dayA.split('/').reverse().join('-'))).map(([day, entries]) => (
                               <div key={day}>
                                ...
                                <ul className="bg-white rounded-b-lg text-left overflow-hidden">
                                  {entries.sort((a,b) => a.timestamp.seconds - b.timestamp.seconds).map((entry) => {
    ...
    <>
      ...
      <div className="min-h-screen flex flex-col bg-gray-50">
        <main className="flex-grow flex flex-col items-center justify-center p-4">
          {/* <img src="/logo.png" alt="Logo da Empresa" className="h-16 w-auto mb-6" /> */}
