...
function App() {
  ...
  const renderActionButtons = () => {
    ...
    switch (userStatus) {
      ...
      case STATUS.ON_BREAK: return <button onClick={() => handleRegister(ENTRY_TYPES.BREAK_END)} className="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-3 md:py-3 md:px-6 rounded-lg shadow-sm">▶️ Retornar do Intervalo</button>;
      default: return <button disabled className="w-full bg-gray-400 text-white font-bold py-2 px-3 md:py-3 md:px-6 rounded-lg">Carregando status...</button>;
    }
  };
    ...
    return (
      <div className="w-full max-w-md">
        <div className="p-6 bg-white rounded-lg shadow-lg">
          ...
          <p className="text-base md:text-lg text-gray-700 mb-6 p-4 h-20 flex items-center justify-center bg-gray-50 rounded-lg">{message}</p>
                  ...
                  {({ open }) => (
                    <>
                      <Disclosure.Button className="flex w-full justify-between rounded-lg bg-blue-100 px-4 py-3 text-left text-sm font-medium text-blue-900 hover:bg-blue-200 focus:outline-none focus-visible:ring focus-visible:ring-blue-500/75">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 text-blue-500 ${open ? 'rotate-180 transform' : ''} transition-transform duration-200`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
