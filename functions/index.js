...
app.post("/api/save-subscription", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: "Inscrição para notificações salva com sucesso!" });
  } catch (error) {
...
app.post("/api/remove-subscription", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: "Inscrição de notificação removida com sucesso!" });
  } catch (error) {
...
exports.sendPunchReminders = onSchedule(
  ...
  async (event) => {
    ...
    for (const doc of employeesSnapshot.docs) {
      ...
      if (
        !employee.workHours ||
        !employee.workHours[todayKey] ||
        !employee.workHours[todayKey].isWorkDay ||
        !employee.pushSubscriptions ||
        employee.pushSubscriptions.length === 0
      ) {
        continue;
      }
      ...
      const lastEntryQuery = await db
        ...
        .limit(1)
        .get();
      const lastEntry = lastEntryQuery.empty
        ? null
        : lastEntryQuery.docs[0].data();
      ...
      const timeSlots = [
        ...
        {
          type: "Início do Intervalo",
          time: schedule.breakStart,
          expectedLast: "Entrada",
        },
        {
          type: "Fim do Intervalo",
          time: schedule.breakEnd,
          expectedLast: "Início do Intervalo",
        },
        {
          type: "Saída",
          time: schedule.exit,
          expectedLast: "Fim do Intervalo",
        },
      ];
      ...
      for (const slot of timeSlots) {
        ...
        if (
          currentTime >= slotTime - 5 &&
          currentTime < slotTime &&
          lastEntryType === slot.expectedLast
        ) {
          notificationPayload = {
      ...
      if (notificationPayload) {
        console.log(
          `Enviando notificação de '${notificationPayload.body}' para ${employee.displayName}`
        );
        ...
        const sendPromises = employee.pushSubscriptions.map((sub) =>
          webpush
            ...
            .catch((err) => {
              console.error(
                `Erro ao enviar para ${sub.endpoint.substring(0, 20)}...:`,
                err.statusCode
              );
            })
        );
